...
from .trainer import RewritingTrainer
import logging
...
def train_rewriting(
    ...
    if output_dir is None:
        output_dir = training_config.output_dir + "/rewriting"
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    
    logger.info(f"Starting rewriting training, output: {output_dir}")
    ...
    # 4. 获取或创建改写 adapter
    if "rewriting" not in model_manager.list_adapters():
        logger.info("Creating rewriting adapter...")
        model_manager.add_adapter(
            "rewriting",
            model_config.rewriting_lora,
            task_type="causal_lm"
        )
    
    # 5. 设置活跃 adapter
    model = model_manager.set_active_adapter("rewriting")
    model.train()
    ...
    model_manager.save_adapter("rewriting", output_dir)
    
    logger.info("[OK] Rewriting training completed!")
...
if __name__ == "__main__":
    ...
    # 训练改写任务
    metrics = train_rewriting(
        model_manager=model_manager,
        train_data_path="data/rewriting/train.json",
        eval_data_path="data/rewriting/eval.json",
        training_config=training_config,
        model_config=model_config,
        output_dir="checkpoints/rewriting"
    )
